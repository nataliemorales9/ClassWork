...
create table User (
    ...
    name    varchar(256) NOT NULL,          -- NON NULL makes this a required value
    phone   varchar(256),                   -- Value is optional since we don't have NON NULL
...
-- You can pretty much ALTER anything, add cols, remove cols, add/remove primary keys or foreign keys. et al

ALTER TABLE User AUTO_INCREMENT = 10293;  -- Set starting uid value instead of starting at 1 and going up by 1
...
create table Customer (  -- create a new table Customer
    ...
    address varchar(256),
    --      (col-in-this)      parent-table(col-in-parent)    - Should be the primary key of the parent
...
-- Customer is dependent of User (established on FOREIGN KEY clause on create table Customer)
...
--                so we will ask the DBMS for uid in User for a specific user
--                    when add an associated Customer table
...
;
-- Like does a wild card search of a table
--
-- % is the wild card character and means 0 to any number of any characters
--
-- starts with: 'word%'
-- ends with: '%word'
-- contains: '%word%'
